...
public final class EntityUtil {
   ...
   */
  public static Resource createResource(Collection<SdkEntity> entities) {
  ...
  public static ResourceBuilder addEntity(ResourceBuilder rb, SdkEntity e) {
    try {
      Method method = ResourceBuilder.class.getDeclaredMethod("add", SdkEntity.class);
      if (method != null) {
        method.setAccessible(true);
        method.invoke(rb, e);
      }
  ...
  public static ResourceBuilder addAllEntity(ResourceBuilder rb, Collection<SdkEntity> e) {
    try {
  ...
  public static Collection<SdkEntity> getEntities(Resource r) {
    try {
      Method method = Resource.class.getDeclaredMethod("getEntities");
      if (method != null) {
        method.setAccessible(true);
        return (Collection<SdkEntity>) method.invoke(r);
      }
    } catch (NoSuchMethodException | IllegalAccessException | InvocationTargetException e) {
      logger.log(Level.WARNING, "Attempting to use entities with unsupported resource", e);
    }
  ...
  public static <T> boolean hasAttributeKey(Collection<SdkEntity> entities, AttributeKey<T> key) {
    return entities.stream()
        .anyMatch(
            e ->
                e.getIdentity().asMap().containsKey(key)
                    || e.getDescription().asMap().containsKey(key));
  }

  /** Decides on a final SchemaURL for OTLP Resource based on entities chosen. */
  @Nullable
  static String mergeResourceSchemaUrl(
      Collection<SdkEntity> entities, @Nullable String baseUrl, @Nullable String nextUrl) {
  ...
  static final RawAttributeMergeResult mergeRawAttributes(
      Attributes base, Attributes additional, Collection<SdkEntity> entities) {
    ...
    ArrayList<SdkEntity> conflicts = new ArrayList<>();
    if (!additional.isEmpty()) {
      additional.forEach(
          (key, value) -> {
            for (SdkEntity e : entities) {
              if (e.getIdentity().get(key) != null || e.getDescription().get(key) != null) {
                // Remove the entity and push all attributes as raw,
                // we have an override.
                conflicts.add(e);
                result.putAll(e.getIdentity()).putAll(e.getDescription());
              }
            }
   ...
   */
  static Collection<SdkEntity> mergeEntities(
      Collection<SdkEntity> base, Collection<SdkEntity> additional) {
    if (base.isEmpty()) {
      return additional;
    }
    if (additional.isEmpty()) {
    ...
    Map<String, SdkEntity> entities = new HashMap<>();
    base.forEach(e -> entities.put(e.getType(), e));
    for (SdkEntity e : additional) {
      if (!entities.containsKey(e.getType())) {
        entities.put(e.getType(), e);
      } else {
        SdkEntity old = entities.get(e.getType());
        ...
        if (old.getSchemaUrl() == null || !old.getSchemaUrl().equals(e.getSchemaUrl())) {
          logger.info(
              "Discovered conflicting entities. Entity ["
                  ...
                  + e.getSchemaUrl()
                  + "] is dropped.");
        } else if (!old.getIdentity().equals(e.getIdentity())) {
  ...
  public static Resource merge(Resource base, @Nullable Resource next) {
    ...
    Collection<SdkEntity> entities = EntityUtil.mergeEntities(getEntities(base), getEntities(next));
